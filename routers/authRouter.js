...
router.post('/reg',
    ...
    async (req, res) => {
        try {
            ...
            const { name, email, password, date, status } = req.body
            ...
            const user = new User({
                name, email, password: hashedPassword, date: dateReg, status: false
            })
    ...
    router.post('/login', async (req, res) => {
        ...
        if (!user) {
            return { status: 'error', error: 'Invalid login', userStatus: user.status }
        }
